...
impl<'tcx> Inliner<'tcx> {
    ...
    fn check_mir_body(
        ...
        while let Some(bb) = work_list.pop() {
            ...
            if let TerminatorKind::DropIfInit { ref place, target, unwind, .. } = term.kind {
...
impl<'tcx> Visitor<'tcx> for CostChecker<'_, 'tcx> {
    ...
    fn visit_terminator(&mut self, terminator: &Terminator<'tcx>, location: Location) {
        let tcx = self.tcx;
        match terminator.kind {
            TerminatorKind::DropIfInit { ref place, unwind, .. } => {
                // If the place doesn't actually need dropping, treat it like a regular goto.
...
impl<'tcx> MutVisitor<'tcx> for Integrator<'_, 'tcx> {
    ...
    fn visit_terminator(&mut self, terminator: &mut Terminator<'tcx>, loc: Location) {
        ...
        match terminator.kind {
            ...
            TerminatorKind::DropIfInit { ref mut target, ref mut unwind, .. }
            | TerminatorKind::DropIf { ref mut target, ref mut unwind, .. } => {
                *target = self.map_block(*target);
